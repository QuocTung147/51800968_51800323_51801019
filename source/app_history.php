...
<body>
    <div class="container">
        ...
        $sql = "SELECT * FROM lichsutai where ID= '$userID' GROUP BY IDUngDung ORDER BY ThoiGianTai DESC Limit $start_from, $record_per_page";
		$res = $conn-> query($sql);
        if($res == TRUE){
            $stt = 1;
            while ($data = $res->fetch_array()) {
                ...
                $sql_select1 = "SELECT Lichhocvaday, Lichthi FROM lich where IDUngDung='$IDUngDung' ";
                $resss = $conn-> query($sql_select1);
                $data3 = $resss->fetch_array();
                $lichhoc = $data3['Lichhocvaday'];
                $lichthi = $data3['Lichthi'];
                ?>
                    <tr class="item">
                        ...
                        <td class=" align-middle"><?=$lichthi?></td>
                    </tr>
                    <?php 
                }          
            }
