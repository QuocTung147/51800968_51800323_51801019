<?php
    require('../../config.php');
    $conn = getDB();
    if(isset($_POST['no']) && isset($_POST['reason'])){
        $id = $_POST['idNo'];
        $reason = $_POST['reason'];
        $query = "UPDATE khoungdung SET TrangThai = 0, LyDo = ? where IDUngDung=?";
        ...
        if ($stmt && $stmt2 && $stmt3) {
            ...
            sendMailDecline($data1['Lastname'],$data1['Firstname'],$data1['Email'], $data2['TenUngDung'],$reason);
